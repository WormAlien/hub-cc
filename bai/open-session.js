...
// на диск: история, куки, localStorage, сессия B.AI).
//
...
//
// 🪤 Главное отличие: у B.AI НЕТ входа через GitHub. Замер `/api/status` 2026-09-15:
...
//
// Чем B.AI отличается от hcnsec, у которого GitHub тоже вырезан:
...
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем bai/profiles/<label>/ (на аккаунт).
...
//      проходишь сам. Затем в ЛК B.AI возьми API-ключ и вставь его в аккаунт кнопкой 🔑
//      на дашборде (или впиши сразу при добавлении).
//   5. Профиль сохраняется автоматически — при следующих открытиях B.AI уже залогинен.
//
// Использование:
//   node bai/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка bai/profiles/<label>/)
...
// routing/ref-codes.default.json (у bai это `6H755H`), пользователь вписывает свой
...
const REGISTER_URL = require('../routing/lib/ref-codes.js').url('bai');
// Ключ уже вписан → сразу баланс, а не логин. Роут именно `/wallet`.
const CONSOLE_URL = 'https://chat.b.ai/wallet';
...
//   share-код друга      → аккаунт B.AI уже создан, панель сразу залогинена;
//   seed:'github'        → файл от GitHub-менеджера, к B.AI отношения не имеет.
function loadImportedSession() {
  try {
    ...
    // готовый GitHub». Для bai он бесполезен (входа через GitHub нет), но и молча
...
// Cloudflare-куки в зачёт не идут — они появляются до всякого входа (у B.AI
...
// (поймано на ar-аккаунтах 2026-08-17, у bai тот же NewAPI-фронт). Кеш профиля
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ B.AI закрыл регистрацию новых аккаунтов (ответ панели) — этот аккаунт создать нельзя.',
  },
];
...
// Сайт сам ушёл на ВНЕШНИЙ вход — у B.AI это Google OAuth (`google_oauth: true`).
...
// прошёл и мы внутри B.AI (консоль/дашборд). Тогда профиль уже сохранён Chromium'ом.
...
async function main() {
  ...
  if (imported && imported.ghSeedOnly) {
    console.log('⚠️  Рядом лежит снимок ТОЛЬКО GitHub-сессии: для bai он бесполезен —');
  ...
  try {
    if (appliedSession) {
      ...
      console.log('✅ Импортированная сессия применена (B.AI уже залогинен).');
    ...
    if (wantRegister) {
      ...
      console.log('   Затем возьми ключ в ЛК B.AI и вставь его кнопкой 🔑 в дашборде.');

      const res = await waitForLogin(page, context);
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на bai закрыта администратором — новый аккаунт не создать.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (B.AI уже залогинен, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await holdOpen(context);
      return;
    }
    ...
    if (!loggedInEarly) console.log('   затем возьми ключ в ЛК B.AI и вставь его кнопкой 🔑 в дашборде.');
