// wisdomsatan/open-session.js
//
// Открывает видимый Chromium с ПЕРСОНАЛЬНЫМ ПРОФИЛЕМ аккаунта (полный профиль на
// диск: история, куки, localStorage, сессия wisdomsatan).
//
// Панель — тот же New API (v0.11.5), что у GoRouter/KKtoken/HCNsec, поэтому механизм
// взят у hcnsec/open-session.js: реф-код в localStorage, кука ЛК, отключённый
// HTTP-кеш, отчёт о белом экране. Отличаются адреса — и путь входа.
//
// 🪤 Главное отличие: у wisdomsatan НЕТ входа через GitHub. Замер `/api/status`
// 2026-09-10: github_oauth=false, oidc_enabled=false, linuxdo_oauth=false,
// telegram_oauth=false, wechat_login=false, discord_oauth=false. Живых путей два —
// пароль и passkey (passkey_login=true). Поэтому вся машинерия GitHub-OAuth из
// исходника ВЫРЕЗАНА, а не оставлена мёртвой: ждать кнопку, которой нет, значит
// висеть десять минут и соврать в лог «таймаут GitHub-логина». По той же причине
// выкинут gh-live-capture и папка `gh-sessions/` — снимать нечего.
//
// 🪤 А вот чем шлюз отличается от эталона hcnsec, откуда взят этот файл: здесь
// регистрация НЕ требует ни кода с почты, ни капчи. Замер 2026-09-10:
// email_verification=false, turnstile_check=false. То есть `POST /api/user/register`
// с одним лишь `{username, password, aff_code}` заводит аккаунт — проверено живьём,
// аккаунт создан, `inviter_id` владельца засчитан. Отсюда два следствия:
//   • ветки с Outlook-ящиком (WS_OL_*) для этого шлюза не нужны и остаются инертными:
//     переменные среды никто не выставляет. Оставлены на случай, если админ панели
//     включит верификацию — тогда достаточно начать их передавать;
//   • шлюз — первый в репозитории кандидат на ПОЛНЫЙ авторег без браузера: капчи нет,
//     почты нет, всё делается четырьмя HTTP-запросами (register → login → token →
//     token/:id/key). Ближайший образец — helpcoder/helpcoder_autoreg.js.
//
// 🪤 Пароль в панели ограничен 8–20 символами (`validate:"min=8,max=20"` в
// model/user.go тега v0.11.5). Двадцать один символ панель молча отвергнет.
//
// Сценарий:
//   1. В дашборде добавляешь аккаунт (email, ключ можно оставить пустым), жмёшь
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем wisdomsatan/profiles/<label>/ (на аккаунт).
//   3. Ключа у аккаунта ещё нет → открывается форма РЕГИСТРАЦИИ по реф-ссылке
//      владельца. Ключ уже вписан → открывается страница баланса (wallet).
//   4. Регистрируешься руками: логин + пароль, и всё — ни кода с почты, ни капчи.
//      Затем в ЛК WisdomSatan возьми API-ключ и вставь его кнопкой 🔑 на дашборде.
//      🪤 Ключ виден ТОЛЬКО в кабинете: `GET /api/token/` и `GET /api/token/:id`
//      отдают его замаскированным (`4YJY**********q3ep`), полный отдаёт лишь
//      `POST /api/token/:id/key`.
//   5. Профиль сохраняется автоматически — при следующих открытиях wisdomsatan уже
//      залогинен.
//
// Использование:
//   node wisdomsatan/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка wisdomsatan/profiles/<label>/)
...
// 🪤 Форма ссылки здесь `/register?aff=`, а НЕ `/sign-up?aff=` восьми соседей-New-API.
// Это не догадка по поколению сборки: по ссылке заведён живой аккаунт 2026-09-10, и в
// его `/api/user/self` приехал `inviter_id` владельца. Панель — New API, значит
// машинерия gorouter'а подходит целиком: код живёт в localStorage `aff`, одного захода
// по ссылке мало, нужна проверка, что он осел (см. openRegisterViaRef).
...
const REGISTER_URL = require('../routing/lib/ref-codes.js').url('wisdomsatan');
...
const CONSOLE_URL = 'https://api.wisdomsatan.club/wallet';
// Корень — источник SITE_HOST для проверки куки ЛК (см. hasSessionCookie).
const ROOT_URL = 'https://api.wisdomsatan.club/';
// Публичная конфигурация панели — на ней держится предполётная проверка.
const STATUS_URL = 'https://api.wisdomsatan.club/api/status';
...
// Источник у файла один: share-код друга (аккаунт wisdomsatan уже создан, панель залогинена).
function loadImportedSession() {
  try {
    ...
    // готовый GitHub». Для wisdomsatan он бесполезен (входа через GitHub нет), но и молча
...
const OL_EMAIL = String(process.env.WS_OL_EMAIL || '');
const OL_PASS = String(process.env.WS_OL_PASS || '');
const OL_SNAPSHOT = String(process.env.WS_OL_SNAPSHOT || '');
...
// `new_api_refresh` (jwt-сборки New API, к ним же относится wisdomsatan) обязан подходить
...
// (поймано на ar-аккаунтах 2026-08-17, у wisdomsatan тот же New-API-фронт). Кеш профиля
...
async function preflight() {
  try {
    ...
    return {
      ...
      site: d.system_name || 'WisdomSatan',
    };
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ wisdomsatan закрыл регистрацию новых аккаунтов (ответ панели) — этот аккаунт создать нельзя.',
  },
];
...
// Подставляем креды в форму входа. Пароль приезжает ПЕРЕМЕННОЙ СРЕДЫ (`WS_LK_PASS`), а не
...
async function prefillLogin(page) {
  const email = String(process.env.WS_LK_EMAIL || '').trim();
  const pass = String(process.env.WS_LK_PASS || '');
...
async function main() {
  ...
  if (imported && imported.ghSeedOnly) {
    console.log('⚠️  Рядом лежит снимок ТОЛЬКО GitHub-сессии: для wisdomsatan он бесполезен —');
  ...
  if (!pre.ok) {
    ...
    if (!pre.passwordLogin) {
      console.log('   ❌ Вход паролем выключен, а других путей у wisdomsatan нет (GitHub/OIDC/Telegram отключены).');
    }
  }
  ...
  try {
    if (appliedSession) {
      ...
      console.log('✅ Импортированная сессия применена (wisdomsatan уже залогинен).');
    ...
    if (wantRegister) {
      ...
      console.log('   Затем возьми ключ в ЛК WisdomSatan и вставь его кнопкой 🔑 в дашборде.');
      ...
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на wisdomsatan закрыта администратором — новый аккаунт не создать.');
    ...
    // 2026-08-21). Здесь этого захода НЕТ, и терять нечего: реф-кода у wisdomsatan не
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (wisdomsatan уже залогинен, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await holdOpen(context);
      return;
    }
