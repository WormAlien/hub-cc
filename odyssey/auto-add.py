    ...
    if mail_kind != "22do":
        ...
        if "--mail-via-proxy" in argv:
            ...
            try:
                ...
                mail = site
        # 🔴 Плюс-алиасы принимаем ПО УМОЛЧАНИЮ, но запас адресов они НЕ множат (замер 25.09).
        # Владелец 17.09: «оказывается с плюсами почту хавает» - и это верно: адрес
        # `careenalo.ng.criern85+jopzut@gmail.com` прошёл целиком. Но вывод «плюс-алиас для
        # площадки ОТДЕЛЬНЫЙ ящик» оказался неверным: Clerk считает базу занятой и отвечает
        # «This email address is already in use» на ЛЮБОЙ новый тег той же базы (25.09:
        # `+lm4maeg145dtsxr` отвергнут, потому что `+722ru9bm5nymv` уже заведён). Режим нужен
        # ровно потому, что 22.do выдаёт плюс-формы наравне с чистыми: 55 из 70 виденных ящиков
        # были с плюсом, и без него мы бы их сами выбрасывали. Но одна БАЗА = один аккаунт.
