...
// — ровно тем же приёмом, что уже работает для итога (`AK_AUTOADD_RESULT {json}`).
//
// 🪤 В файл лога эту строку НЕ пишем: она служебная, для stdout-контракта с бэкендом.
// В `logs/aikeysapi-autoadd.log` человек читает обычные строки.
...
function stage(name, { i, count, note } = {}) {
    ...
    console.log('AK_STAGE ' + JSON.stringify(payload));
}
...
async function checkSavedBalance(rec) {
    const url = `http://127.0.0.1:8200/__switch/api/ak/balance?api_key=${encodeURIComponent(rec.api_key)}`;
...
// `noreply@aikeysapi.com`, а instanttempemail — только ДИСПЛЕЙ-ИМЯ `ZhiFlow · 智流AI`,
// без адреса вовсе. Фильтр по `@aikeysapi.com` молча выбрасывал письмо, оно лежало в
...
const PANEL_BRAND_RE = /zhiflow|智流|aikeysapi/i;
...
// а прокси нужен только для запросов к ZhiFlow. Значит порядок можно развернуть:
...
async function acquireProxyFor(email, { noProxy, index, count }) {
    ...
    const key = `aikeysapi:${String(email).toLowerCase()}`;
    const tried = new Set();
...
//
// 🪤 ОДНОЙ КУКИ НЕ ХВАТАЕТ, и это неочевидно: панель ZhiFlow — SPA на New API, и она
