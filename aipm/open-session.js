...
//
// Чем aipm отличается (замер `/api/status`):
...
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем aipm/profiles/<label>/ (на аккаунт).
...
//
// Использование:
//   node aipm/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка aipm/profiles/<label>/)
...
const REGISTER_URL = require("../routing/lib/ref-codes.js").url("aipm");
// Ключ уже вписан → сразу баланс, а не логин.
const CONSOLE_URL = 'https://emtf.aipm9527.online/wallet';
// Корень нужен для прогрева перед регистрацией (см. openRegisterViaRef).
const ROOT_URL = 'https://emtf.aipm9527.online/';
...
const ghCapture = require('../routing/lib/gh-live-capture.js').makeCapture({
  label,
  moduleDir: __dirname,
  poolFile: path.join(__dirname, '..', 'routing', 'aipm-sessions.json'),
});
...
//   share-код друга      → аккаунт aipm уже создан, GitHub/aipm сразу залогинены;
//   seed:'github'        → только GitHub-куки, аккаунта aipm ещё НЕТ (см. seededGithub).
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ aipm закрыл регистрацию новых аккаунтов (ответ сайта) — этот аккаунт создать нельзя.',
  },
  {
    code: 'git_token',
...
// GitHub-вход прошёл и мы внутри aipm (консоль/дашборд). Тогда профиль уже
...
async function main() {
  ...
  try {
    if (appliedSession && !seededGithub) {
      ...
      console.log('✅ Импортированная сессия применена (GitHub/aipm уже залогинены).');
    ...
    if (wantRegister) {
      ...
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на aipm закрыта администратором — новый аккаунт не создать.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (GitHub/aipm уже залогинены, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await ghCapture.holdOpen(context); // держим открытым, закрытие — вручную
      return;
    }
