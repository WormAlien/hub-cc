...
def mail_key(addr):
    ...
    ящики, и склеивать их нельзя. Плюс-часть отбрасываем у всех, и это замер 25.09: Clerk
    плюс-адрес ПРИНИМАЕТ (17.09), но отдельным ящиком не считает - база, у которой уже есть
    аккаунт, отвергается на любом новом теге («This email address is already in use»).
    Значит **база даёт ровно один аккаунт**, и ключ занятости - именно она.
    """
...
async def pick_22do_gmail(page, log, tries=RANDOM_MAX_TRIES, rounds=10, allow_plus=False):
    ...
    # 🔴 Ключ занятости - БАЗА ящика, а НЕ полный адрес, и это замер 25.09. Плюс-алиас Clerk
    # принимает (17.09), но отдельным ящиком не считает: база `efymxn39531c@outlook.com` заведена
    # тегом `+722ru9bm5nymv` (аккаунт od_1789637688866_13, 17.09), а 25.09 ЕЁ ЖЕ новый тег
    # `+lm4maeg145dtsxr` получил 403 «This email address is already in use». То же на gmail:
    # `tro.sk.ykr.eie5.16+55fb` заведён, а `trosk.ykre.i.e.516+zcn230` отвергнут. Значит
    # **база даёт РОВНО ОДИН аккаунт**, и плюс-алиасы запаса адресов не множат.
    used = used_addresses()
    if used:
        log("ПОЧТА", f"занятых баз ящиков в пуле и мета-файлах: {len(used)} - выданный сверяю с ними")
    busy_bases = []
    for rnd in range(1, rounds + 1):
        ...
        busy = mail_key(addr) in used
        if clean_22do_address(addr, allow_plus) and not busy:
            log("ПОЧТА", f"✅ адрес готов: {addr}")
            return addr
        if busy:
            # 🔴 Тот же ЯЩИК под другим тегом. 22.do крутит базу повторно, меняя только
            # плюс-часть, а Clerk считает такой адрес уже занятым - «This email address is
            # already in use». Снаружи это читается как отказ регистрации, а не как «ящик
            # выдан повторно». Замер 16.09 (точки в gmail) и подтверждение 25.09 (плюс-алиасы
            # outlook): база пула `efymxn39531c@outlook.com` уже заведена, новый тег отвергнут.
            busy_bases.append(addr)
            log("ПОЧТА", f"адрес «{addr}» - та же база ящика, что уже занята "
                         f"({mail_key(addr)}) - раунд {rnd} из {rounds} заново")
            continue
        ...
        log("ПОЧТА", f"адрес «{addr or 'не прочитался'}» не беру (домен не из допущенных или "
                     f"плюс запрещён флагом) - раунд {rnd} из {rounds} заново")
    log("ПОЧТА", f"❌ за {rounds} раундов годного ящика не нашлось"
                 + (f"; 22.do выдаёт занятые базы: {', '.join(busy_bases[:6])}" if busy_bases else ""))
    return ""
