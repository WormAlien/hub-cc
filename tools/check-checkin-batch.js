...
// входа без кнопки GitHub), 5 (шлюз отверг OAuth), 6 (край не ответил), 7 (прокси непригоден).
// Коды 2 и 3 — про конкретный аккаунт, и вставать по ним нельзя: два дохлых аккаунта подряд
// заперли бы остальные девять живых.
//
...
console.log('\n3. предохранитель по отказам подряд');
{
    ...
    check([4, 5, 6, 7].every(c => codes.includes(c)),
        `считаются коды «бьёт по IP»: 4, 5, 6, 7 (найдено ${codes.join(', ') || '—'})`);
