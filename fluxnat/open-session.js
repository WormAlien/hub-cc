// fluxnat/open-session.js
//
// Открывает видимый Chromium с ПЕРСОНАЛЬНЫМ ПРОФИЛЕМ аккаунта (полный профиль
// на диск: история, куки, localStorage, сессии GitHub + FluxRouter).
//
...
//
// Чем FluxRouter отличается — ЗАМЕРЕНО живым `GET /api/status` 2026-09-13:
//   • вход через GitHub-OAuth есть (`github_oauth: true`), плюс discord и telegram;
//     парольный вход на регистрации ВЫКЛЮЧЕН (`password_register_enabled: false`) —
//     почтой завести аккаунт нельзя, только через OAuth;
//   • ✅ капчи НЕТ (`turnstile_check: false`) — вопреки ожиданию «по фронту NewAPI».
//     Значит авто-заведение (⚡, как у JustWoker) тут принципиально возможно: мешает
//     не капча, а то, что единственный вход на регистрации — GitHub-аккаунт из пула.
//     Пока сценарий не написан, скрипт открывает окно и ждёт человека — он ничего
//     не регистрирует сам;
//   • `email_verification: false` — почту подтверждать не надо;
//   • `checkin_enabled: true` — у шлюза есть ежедневный чек-ин, то есть пополнение
//     без денег; у hcnsec это «единственный способ пополнения» ([[ABUSE HUB]]).
//     Стоит проверить, что он реально капает, до того как вкладка станет основной.
//
// 🪤 Два пункта НЕ проверены живым заходом и выводятся по аналогии с фронтом NewAPI:
// страница баланса `/wallet` и поведение реф-ссылки. Подтвердить при первом заходе.
//
// Сценарий:
//   1. В дашборде добавляешь аккаунт (email, ключ можно оставить пустым), жмёшь
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем fluxnat/profiles/<label>/ (на аккаунт).
//   3. Ключа у аккаунта ещё нет → открывается РЕГИСТРАЦИЯ по рефке владельца.
//      Ключ уже вписан → открывается страница баланса (wallet).
//   4. В консоли FluxRouter возьми API-ключ и вставь его в аккаунт кнопкой 🔑
//      на дашборде (или впиши сразу при добавлении).
//   5. Профиль сохраняется автоматически — при следующих открытиях GitHub и
//      FluxRouter уже залогинены.
//
// Использование:
//   node fluxnat/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка fluxnat/profiles/<label>/)
...
// 🪤 url() отдаёт `null`, пока шлюза нет в `SHAPES` (routing/lib/ref-codes.js):
// тогда реф-ссылки нет вовсе, и без фолбэка регистрация поехала бы на null-адрес.
// Фолбэк — корень сайта, ровно как ведёт себя сам url() без кода.
const REGISTER_URL = require("../routing/lib/ref-codes.js").url("fluxnat") || 'https://llm.fluxnat.dev/';
// Ключ уже вписан → сразу баланс, а не логин.
const CONSOLE_URL = 'https://llm.fluxnat.dev/wallet';
// Корень нужен для прогрева перед регистрацией (см. openRegisterViaRef).
const ROOT_URL = 'https://llm.fluxnat.dev/';
...
const ghCapture = require('../routing/lib/gh-live-capture.js').makeCapture({
  label,
  moduleDir: __dirname,
  poolFile: path.join(__dirname, '..', 'routing', 'fluxnat-sessions.json'),
});
...
//   share-код друга      → аккаунт FluxRouter уже создан, GitHub/FluxRouter сразу залогинены;
//   seed:'github'        → только GitHub-куки, аккаунта FluxRouter ещё НЕТ (см. seededGithub).
...
// (поймано на ar-аккаунтах 2026-08-17, у gorouter/tabi/fluxnat тот же NewAPI-фронт).
...
// Ответы FluxRouter (NewAPI) на неудачный GitHub-вход. Раньше скрипт знал только
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ FluxRouter закрыл регистрацию новых аккаунтов (ответ сайта) — этот аккаунт создать нельзя.',
  },
  {
    code: 'git_token',
...
// GitHub-вход прошёл и мы внутри FluxRouter (консоль/дашборд). Тогда профиль уже
...
async function main() {
  ...
  try {
    if (appliedSession && !seededGithub) {
      ...
      console.log('✅ Импортированная сессия применена (GitHub/FluxRouter уже залогинены).');
    ...
    if (wantRegister) {
      ...
      console.log('   затем возьми ключ в консоли FluxRouter и вставь его кнопкой 🔑 в дашборде.');

      const res = await waitForLogin(page, context);
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на FluxRouter закрыта администратором — новый аккаунт не создать.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (GitHub/FluxRouter уже залогинены, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await ghCapture.holdOpen(context); // держим открытым, закрытие — вручную
      return;
    }
    ...
    if (!loggedInEarly) console.log('   затем возьми ключ в консоли FluxRouter и вставь его кнопкой 🔑 в дашборде.');
